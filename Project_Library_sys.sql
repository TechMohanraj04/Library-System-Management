CREATE DATABASE library_sys_mang;

use library_sys_mang;
...
-- showing all the inserted tables..
...
from library_sys_mang.issued_status
group by 1
...
with book_cnts as
(
select b.isbn,b.book_title, count(sta.issued_id)  as count_of_issued_books from books as b
join issued_status as sta
on b.isbn=sta.issued_book_isbn
group by 1,2
)
...
select b.category,sum(rental_price) as total_books_issued,count(*) as count_issued from books as b
join issued_status as sta
on b.isbn=sta.issued_book_isbn
group by 1;
...
insert into members(member_id,member_name,member_address,reg_date)
values('C121','Mohan','122 Per St','2025-01-01'),
('C122','Naren','113 soler St','2025-02-01');
...
select e.emp_name,b.* ,count(issued_emp_id) as no_book_issued from 
issued_status as is_sta
join employees as e
on is_sta.issued_emp_id=e.emp_id
join branch as b
